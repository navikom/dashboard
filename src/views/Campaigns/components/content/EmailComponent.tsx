...
const EmailFormDialog = (props: {open: boolean, handleClose: () => void, onSend: (email: string) => void}) => {
 const constraints = {
  email: {
   presence: {
 ...
 return (
   <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
    ...
    <DialogActions>
     ...
     <Button disabled={errors !== undefined || email.length === 0} onClick={onSendHandler} color="primary">
...
const EmailComponent = (props: {store: IContentEmailView}) => {
 ...
 const onAttachmentInput = (index: number) => (e: React.ChangeEvent<HTMLInputElement> | string) => {
  store.attachments[index].setUrl(typeof e === "string" ? store.attachments[index].url + e : e.target.value);
 };
        ...
        <Grid item xs={12} sm={12} md={6}>
         {
          store.htmlFile && (
            <Button
              ...
              variant="outlined" onClick={() => setTestLetterDialogOpen(true)} color="primary">
         ...
         <div className={extraClasses.letterWrapper}>
          ...
          <div className={extraClasses.letterCover} onClick={store.openEditor} />
         </div>
        </Grid>
       </Grid>
      </Grid>
        ...
        <Grid item xs={12} sm={12} md={6}>
         {
          store.attachments.map((attachment: IAttachment, i: number) => (
            <FormControl key={i} fullWidth className={extraClasses.marginBottom5}>
             <InputWithIcon
               divider={{className: extraClasses.divider}}
               ...
               endAdornments={[
                ...
                {component: <Delete />, onClick: () => store.deleteAttachment(i)}
               ]}
             />
            </FormControl>
          ))
         ...
         <Button
           ...
           variant="outlined" onClick={store.addAttachment} color="primary">
