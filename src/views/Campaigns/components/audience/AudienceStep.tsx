...
const SegmentRows = observer((props: ISegmentRow) => {
 ...
 const buttonTitle = `${Dictionary.value(include ? "include" : "exclude")} ${Dictionary.defValue(DictionaryService.keys.segment)}`;
 return (
   <div className={extraClasses.root}>
    ...
    <Grid container item direction="row">
     <Typography variant="subtitle2" className={centerNote}> </Typography>
...
const Reachable = observer(() => {
 ...
 const centerNote = classNames(classes.note, classes.center, extraClasses.label, extraClasses.highlight);
       ...
       <Grid item xs={12} sm={12} md={6}>
        <BootstrapInput
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            store.setName(e.target.value.length ? e.target.value : undefined)}
        />
       </Grid>
      </Grid>
        ...
        <FormControl component="fieldset">
         <RadioGroup onChange={() => store.switchMultipleSegments()}>
          <FormControlLabel
            ...
            label={Dictionary.defValue(DictionaryService.keys.usersInSingleSegment)}
          />
          <FormControlLabel
            checked={store.multipleSegments}
            control={<Radio color="primary" />}
            label={Dictionary.defValue(DictionaryService.keys.usersInMultipleSegments)}
          />
         </RadioGroup>
        </FormControl>
       </Grid>
         ...
         <Grid container item direction="row" className={extraClasses.container}>
          ...
          <Grid container item xs={12} sm={12} md={6}>
           <Grid item xs={12} sm={8} md={10}>
            <FormControl fullWidth>
             <CustomSelect
               value={store.includeSegments.length > 0 ? store.includeSegments[0].segmentId : ""}
